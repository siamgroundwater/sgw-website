...
const worksheetExampleByLocale: Record<LocalizedLocale, string> = {
  th: 'ตัวเลขเริ่มต้นเป็นตัวอย่าง เปลี่ยนเป็นค่าที่วัดจากบ่อของคุณก่อนนำผลไปใช้',
  en: 'The starting values are examples. Replace them with measurements from your well before using the result.',
  zh: '初始数值仅为示例。使用结果前，请填写您水井的实测数据。',
  ja: '初期値は入力例です。結果を使う前に、ご自身の井戸の測定値へ変更してください。',
}

const copyByLocale: Record<LocalizedLocale, CourseCopy> = {
  th: {
    summary: { duration: 'เวลาเรียน', durationValue: '35–45 นาที', level: 'ระดับ', levelValue: 'เจ้าของบ่อ–ช่างเทคนิค', cases: 'กรณีศึกษา', casesValue: '5 สถานการณ์จำลอง' },
    ...
    workflow: {
      ...
      steps: [
        { title: 'ตั้งค่าฐาน', detail: 'รวบรวมแบบบ่อ รายงานเจาะ รุ่นปั๊ม ผลสูบทดสอบ และผลตรวจน้ำเดิม', output: 'ค่าปกติที่ใช้อ้างอิง' },
        ...
        { title: 'ทดสอบยืนยัน', detail: 'วัดค่าหลักและสูบทดสอบซ้ำหลังงาน ก่อนกำหนดปั๊มหรืออัตราสูบใหม่', output: 'หลักฐานว่าดีขึ้นจริง' },
      ],
      principleTitle: 'หลักสำคัญ', principle: 'เทียบค่าก่อนและหลังที่อัตราสูบ ระยะเวลาสูบ และเงื่อนไขใกล้กัน อย่าสรุปสาเหตุจากตัวเลขเพียงค่าเดียว',
    },
    cases: {
      eyebrow: 'เรียนจากหลักฐาน', title: '5 กรณีศึกษาแบบอาการ → หลักฐาน → การยืนยัน', intro: 'ตัวอย่างจำลองสำหรับฝึกแยกอาการ หลักฐาน และสิ่งที่ต้องตรวจยืนยัน ไม่ใช่ผลวินิจฉัยหรือค่ารับประกันสำหรับบ่อจริง',
      ...
      items: [
        { title: 'บ่อโรงงานผลิตน้ำลดลง', setting: 'บ่อเดิมมีข้อมูลทดสอบและเดินเครื่องทุกวัน', symptom: 'อัตราการไหลลด 35% แต่ปั๊มยังทำงาน', evidence: 'ระดับสถิตใกล้เดิม แต่ระยะน้ำลดเพิ่มและการคืนตัวช้าลง', finding: 'ในตัวอย่างนี้ ควรตรวจการอุดตันบริเวณรับน้ำก่อน แต่ยังต้องตรวจยืนยันสาเหตุ', action: 'ตรวจชนิดตะกรันหรือตะกอนก่อนเลือกวิธีฟื้นฟูบ่อ ยังไม่เพิ่มขนาดปั๊ม', verify: 'สูบทดสอบซ้ำที่อัตราและระยะเวลาใกล้เดิม เปรียบเทียบอัตราสูบหารด้วยระยะน้ำลด (specific capacity)', lesson: 'ระดับน้ำก่อนสูบใกล้เดิม แต่ขณะสูบน้ำลดมากขึ้น ควรตรวจประสิทธิภาพบ่อก่อน' },
        { title: 'บ่อเกษตรมีทรายหลังเพิ่มอัตราสูบ', setting: 'เปลี่ยนปั๊มใหม่กำลังสูงกว่าเดิม', symptom: 'น้ำขุ่นและมีทรายช่วงสูบต่อเนื่อง', evidence: 'ทรายเพิ่มตามอัตราสูบ ความลึกบ่อลด และแรงดันแกว่ง', finding: 'การสูบเกินสมรรถนะร่วมกับปัญหากรวดกรุ/ตะแกรง', action: 'ลดอัตราสูบ ตรวจความลึกและสภาพตะแกรง แล้วออกแบบแก้ไข', verify: 'เก็บตัวอย่างทรายตามเวลาขณะสูบทดสอบแบบเพิ่มอัตราสูบเป็นขั้น', lesson: 'ปั๊มใหญ่ขึ้นไม่ได้ทำให้บ่อให้น้ำได้มากขึ้นอย่างปลอดภัยเสมอ' },
        ...
        { title: 'ความเค็มเพิ่มเฉพาะช่วงสูบหนัก', setting: 'พื้นที่เสี่ยงน้ำกร่อย ใช้บ่อหลายช่วงเวลา', symptom: 'EC/TDS เพิ่มเมื่อเดินปั๊มนาน', evidence: 'ค่าลดลงหลังพักบ่อ แต่กลับสูงเมื่อเพิ่มอัตราสูบ', finding: 'มีความเสี่ยงดึงน้ำเค็มหรือเกิดการไหลข้ามชั้น ต้องยืนยันโครงสร้างบ่อ', action: 'ลดอัตราสูบ เก็บตัวอย่างน้ำเป็นช่วงเวลา และตรวจท่อกรุกับช่วงรับน้ำ', verify: 'ติดตาม EC พร้อมอัตราสูบและระดับน้ำหลายรอบ', lesson: 'ต้องดูความสัมพันธ์กับเวลาและอัตราสูบ ไม่ใช่ผลตัวอย่างครั้งเดียว' },
    ...
    worksheet: {
      ...
      pathways: {
        ...
        aquifer: { title: 'ตรวจการเปลี่ยนแปลงระดับชั้นน้ำก่อน', text: 'ระดับสถิตลึกขึ้นอย่างมีนัยสำคัญ อาจเกี่ยวกับฤดูกาล การเติมน้ำ หรือการสูบโดยรอบ', next: 'ตรวจระดับบ่อข้างเคียงและประวัติฝน สูบทดสอบด้วยอัตราที่ควบคุม พร้อมวัดการคืนตัวของระดับน้ำ' },
        well: { title: 'ตรวจประสิทธิภาพบ่อและช่วงรับน้ำ', text: 'น้ำไหลน้อยลงและระดับน้ำลดขณะสูบมากขึ้น ทั้งที่ระดับก่อนสูบใกล้เดิม อาจมีความสูญเสียในบ่อเพิ่ม ต้องตรวจยืนยัน', next: 'เทียบอัตราสูบหารด้วยระยะน้ำลด ตรวจตะกรันหรือตะกอน แล้ววางแผนตรวจเชิงลึกก่อนฟื้นฟู' },
        equipment: { title: 'ตรวจปั๊ม ท่อ วาล์ว และไฟฟ้าก่อน', text: 'น้ำไหลน้อยลง แต่ระดับก่อนสูบและระยะน้ำลดใกล้เดิม ควรเริ่มตรวจปั๊ม ท่อ วาล์ว และไฟฟ้า', next: 'บันทึกแรงดัน กระแสไฟ ความดัน ตำแหน่งวาล์ว และตรวจรอยรั่ว/การสึก' },
    ...
    prevention: {
      ...
      recordsTitle: 'ติดตามแนวโน้ม ไม่ใช่แค่ผ่าน/ไม่ผ่าน', records: [
        { title: 'สมรรถนะบ่อ', detail: 'บันทึกอัตราไหล ระยะน้ำลด และอัตราสูบหารด้วยระยะน้ำลด โดยใช้เงื่อนไขทดสอบใกล้กัน' },
    ...
    quiz: {
      ...
      questions: [
        ...
        { question: 'ระดับน้ำสถิตใกล้เดิม แต่ระดับขณะสูบลึกขึ้นมาก บอกอะไรได้ดีที่สุด?', choices: ['ความสูญเสียในบ่ออาจเพิ่มขึ้น', 'ยืนยันว่ามอเตอร์เสีย', 'ยืนยันว่าชั้นน้ำแห้งถาวร'], correct: 0, explanation: 'หากระดับน้ำลดขณะสูบมากขึ้นภายใต้เงื่อนไขใกล้กัน ให้ตรวจประสิทธิภาพบ่อและช่วงรับน้ำก่อน แต่ยังต้องยืนยันสาเหตุ' },
    ...
    next: { title: 'ต้องการนำข้อมูลไปคำนวณต่อ?', text: 'คำนวณระยะน้ำลด ค่าช่วยเปรียบเทียบสมรรถนะบ่อ ปริมาตรถัง และพลังงาน หรือส่งข้อมูลหน้างานให้ทีมช่วยวางแผนตรวจ', calculator: 'เปิดเครื่องมือคำนวณ', contact: 'ปรึกษาทีมงาน' },
  },
  en: {
    summary: { duration: 'Study time', durationValue: '35–45 minutes', level: 'Level', levelValue: 'Owners–technicians', cases: 'Case studies', casesValue: '5 teaching scenarios' },
    ...
    ], principleTitle: 'Core principle', principle: 'Compare before-and-after values at similar pumping rates, durations and conditions. Do not diagnose from one number alone.' },
    cases: { eyebrow: 'Evidence-based practice', title: '5 symptom → evidence → verification cases', intro: 'Illustrative examples for separating symptoms, evidence and checks still needed. They are not diagnoses or guarantees for a real well.', labels: { setting: 'Setting', symptom: 'Symptom', evidence: 'Key evidence', finding: 'Finding', action: 'Corrective action', verify: 'Verification', lesson: 'Lesson' }, items: [
      { title: 'Factory well loses output', setting: 'Daily-use well with historic test data', symptom: 'Flow down 35%; pump still runs', evidence: 'Static level similar, but drawdown increased and recovery slowed', finding: 'In this example, check intake clogging first; measurements alone do not confirm the cause', action: 'Identify scale or sediment before choosing rehabilitation; do not upsize the pump first', verify: 'Repeat at a similar rate and duration; compare pumping rate divided by drawdown (specific capacity)', lesson: 'Similar pre-pumping level but a greater drop during pumping means well performance should be checked first' },
    ...
    worksheet: { eyebrow: 'Field worksheet', title: 'Compare baseline with current performance', intro: 'Use readings from similar pumping rates, durations and conditions. The result helps choose what to check and discuss with a technician.', baseline: 'Baseline', current: 'Current', flow: 'Flow rate', staticLevel: 'Static level below wellhead', pumpingLevel: 'Pumping level below wellhead', sandFlag: 'Abnormal sand or turbidity observed', qualityFlag: 'Clear water-quality/salinity change', resultTitle: 'First-priority pathway', metrics: { flowChange: 'Flow change', staticShift: 'Static level deepened', drawdownChange: 'Drawdown change' }, pathways: {
      sand: { title: 'Prioritize sand and well integrity', text: 'Reduce flow and collect timed samples before sand damages the pump and system.', next: 'Measure sand, verify well depth and consider camera inspection when supported.' }, quality: { title: 'Prioritize water quality', text: 'Isolate sensitive uses and confirm with correct sampling and laboratory analysis.', next: 'Record EC/TDS, pH, turbidity, runtime and use-specific laboratory parameters.' }, aquifer: { title: 'Investigate aquifer-level change first', text: 'A material deepening of static level can reflect season, recharge or surrounding pumping.', next: 'Review nearby levels/rainfall and run a controlled pumping/recovery test.' }, well: { title: 'Investigate well and intake efficiency', text: 'Less flow with a greater water-level drop during pumping, while the pre-pumping level is similar, may indicate increasing well loss. Confirm the cause.', next: 'Compare pumping rate divided by water-level drop, and inspect scale or sediment before planning rehabilitation.' }, equipment: { title: 'Check pump, pipe, valve and electrical system first', text: 'Flow fell while the pre-pumping level and the drop during pumping stayed similar. Start with pump, pipe, valve and electrical checks.', next: 'Record pressure, current, voltage and valve position; inspect leakage and wear.' }, stable: { title: 'No strong change in the entered values', text: 'Continue trend monitoring and confirm measurements used the same method and conditions.', next: 'Record monthly performance, runtime, quality and surrounding events.' },
    ...
    prevention: { eyebrow: 'Prevent recurrence', title: 'Small records make later diagnosis much faster', intro: 'Consistent baselines expose deterioration before shutdown and help target repair.', checklistTitle: 'Well-record checklist', complete: 'Complete—ready to build a baseline history', remaining: 'Items still to collect', checklist: ['Construction log, depth, screen intervals and gravel-pack record', 'Pump model, setting depth, curve and protection settings', 'Flow, pressure, static/pumping levels and recovery', 'Voltage, per-phase current, runtime and kWh/m³', 'Water results with date, sampling point and sample condition', 'Sand, rehabilitation, pump repair and incident history'], recordsTitle: 'Track trends, not only pass/fail', records: [{ title: 'Well performance', detail: 'Record flow, water-level drop and pumping rate divided by that drop, using comparable test conditions.' }, { title: 'Energy', detail: 'Use kWh/m³ with water level and pressure to expose lost efficiency.' }, { title: 'Water quality', detail: 'Plot results over time and runtime because some values drift while pumping.' }, { title: 'Events', detail: 'Record drought, flood, construction, nearby wells and valve changes.' }] },
    ...
    next: { title: 'Continue with your field data', text: 'Calculate water-level drop, well-comparison values, tank volume and energy, or share site observations with our team.', calculator: 'Open calculator suite', contact: 'Contact the team' },
  },
  zh: {
    summary: { duration: '学习时间', durationValue: '35–45 分钟', level: '适合人群', levelValue: '井主–技术人员', cases: '案例', casesValue: '5 个教学情境' }, outcomesTitle: '完成后，您将能够', outcomes: ['区分水井、泵、管路与电气故障', '在决定维修前选择必要的现场数据', '系统阅读流量、水位与降深趋势', '识别必须停机并升级处理的信号'], jumpLabel: '跳至', chapters: { triage: '症状筛查', workflow: '诊断顺序', cases: '案例学习', worksheet: '评估表', prevention: '预防', quiz: '知识检测' }, visualAlt: { comparison: '健康地下水井与堵塞产砂井的剖面对比', field: '技术人员测量水位、电气、流量并采集水样' }, visualCaption: { comparison: '出水量下降不一定是泵故障；滤管、砾料、水层水位和输水系统都要分别检查。', field: '可靠诊断需要在已知运行条件下同时采集多组数据。' },
    ...
    worksheet: { eyebrow: '现场评估表', title: '对比基准与当前性能', intro: '使用抽水流量、时长和条件相近的读数。结果帮助您选择检查重点，并与技术人员沟通。', baseline: '基准值', current: '当前值', flow: '流量', staticLevel: '井口以下静水位', pumpingLevel: '井口以下动水位', sandFlag: '发现异常砂或浑浊', qualityFlag: '水质/盐度明显变化', resultTitle: '优先检查路径', metrics: { flowChange: '流量变化', staticShift: '静水位加深', drawdownChange: '降深变化' }, pathways: {
      sand: { title: '优先处理出砂和井体完整性', text: '降低流量并定时取样，避免砂损坏泵和系统。', next: '测砂量、核实井深，并按证据考虑井下摄像。' }, quality: { title: '优先处理水质', text: '暂停敏感用途，并通过规范采样和实验室确认。', next: '记录 EC/TDS、pH、浊度、运行时间和用途相关指标。' }, aquifer: { title: '先调查水层水位变化', text: '静水位明显加深可能与季节、补给或周边抽水有关。', next: '查看邻井和降雨，并进行受控抽水/恢复试验。' }, well: { title: '调查井和进水段效率', text: '抽水前水位相近，但出水量减少、抽水时水位下降更多，可能表示井内损失增加。仍需确认原因。', next: '比较抽水流量除以水位降深，并检查结垢或沉积物，再安排深入检查与修复。' }, equipment: { title: '先查泵、管路、阀门和电气', text: '流量下降而水位与降深相近。', next: '记录压力、电流、电压和阀位，检查泄漏和磨损。' }, stable: { title: '输入值未显示强烈变化', text: '继续趋势监测，并确认测量方法与条件一致。', next: '每月记录性能、运行时间、水质和周边事件。' },
    ...
    prevention: { eyebrow: '预防复发', title: '少量记录能大幅加快诊断', intro: '一致的基准能在停机前发现恶化，并让维修更有针对性。', checklistTitle: '水井资料清单', complete: '已完整—可以建立基准历史', remaining: '仍需收集', checklist: ['成井记录、井深、滤管区间和砾料记录', '泵型、安装深度、曲线和保护设定', '流量、压力、静/动水位和恢复', '电压、各相电流、运行时间和 kWh/m³', '水质结果、日期、采样点和样品状态', '出砂、洗井、泵维修和故障历史'], recordsTitle: '看趋势，而不只看合格/不合格', records: [{ title: '井性能', detail: '在相近试验条件下记录流量、水位下降量，以及流量除以下降量的值。' }, { title: '能耗', detail: '结合水位与压力分析 kWh/m³。' }, { title: '水质', detail: '按时间和运行时长绘图。' }, { title: '事件', detail: '记录干旱、洪水、施工、邻井和阀门变化。' }] },
  ...
  ja: {
    summary: { duration: '学習時間', durationValue: '35～45分', level: '対象', levelValue: '井戸所有者～技術者', cases: 'ケース', casesValue: '学習用の想定事例 5件' }, outcomesTitle: '学習後にできること', outcomes: ['井戸・ポンプ・配管・電気の不具合を切り分ける', '修理判断前に必要な現場データを選ぶ', '流量・水位・水位低下量の傾向を読む', '停止・専門家対応が必要な兆候を判断する'], jumpLabel: '章へ移動', chapters: { triage: '症状の切り分け', workflow: '診断手順', cases: 'ケーススタディ', worksheet: '評価シート', prevention: '予防', quiz: '理解度確認' }, visualAlt: { comparison: '健全な地下水井と目詰まり・揚砂井の断面比較', field: '水位、電気、流量を測定し採水する技術者' }, visualCaption: { comparison: '揚水量低下は必ずしもポンプ故障ではありません。スクリーン、砂利充填、帯水層水位、送水系統を分けて確認します。', field: '確かな診断は、既知の運転条件で複数の測定値を同時に集めることから始まります。' },
    ...
    workflow: { eyebrow: '診断プロセス', title: '低コスト測定から深い証拠へ', intro: '不要なポンプ引上げや根拠のない井戸洗浄を減らし、前後比較を可能にします。', outputLabel: '成果', steps: [{ title: '基準値を作る', detail: '井戸台帳、完成記録、ポンプ、揚水試験、過去水質を集めます。', output: '正常時の参照値' }, { title: '症状を確認', detail: '計器、弁、漏れを確認し、発生時期・条件を特定します。', output: '再現できる症状' }, { title: '系統を測定', detail: '流量、圧力、電圧、電流、電力量を同時に記録します。', output: 'ポンプ・配管・電気の切分け' }, { title: '水位を測定', detail: '既知流量で静水位、動水位、水位低下量、回復を測定します。', output: '帯水層・井戸・ポンプの切分け' }, { title: '水と砂を確認', detail: '時間別試料、濁り、砂、水質をリスクに応じて確認します。', output: '水質・沈殿の証拠' }, { title: '必要に応じ詳細調査', detail: '証拠に基づきポンプ引上げ、深度確認、カメラ等を実施します。', output: '物理原因の確認' }, { title: '原因に合わせ修正', detail: '修理、井戸洗浄、揚水量調整、水処理を原因別に選びます。', output: '作業範囲と判定基準' }, { title: '性能を検証', detail: '主要値と揚水試験を再実施してからポンプを選定します。', output: '改善の証拠' }], principleTitle: '基本原則', principle: '揚水量、時間、条件が近い測定値で前後を比べます。1つの数値だけで原因を決めないでください。' },
    cases: { eyebrow: '証拠から学ぶ', title: '5つの症状 → 証拠 → 検証ケース', intro: '症状、根拠、確認が必要な点を分けて考えるための想定例です。実際の井戸の診断や保証ではありません。', labels: { setting: '背景', symptom: '症状', evidence: '重要証拠', finding: '所見', action: '対策', verify: '検証', lesson: '学び' }, items: [
      { title: '工場井戸の揚水量低下', setting: '過去試験のある常用井', symptom: '流量35%低下、ポンプは運転', evidence: '静水位は同程度、低下量増大、回復遅延', finding: 'この例では取水部の目詰まりを先に調べます。測定値だけで原因は確定できません', action: '付着物や堆積物の種類を確認してから回復方法を選び、先にポンプを大型化しません', verify: '近い揚水量と時間で再試験し、揚水量を水位低下量で割った値（比湧出量）を比較', lesson: '揚水前の水位が同程度でも、揚水中により大きく下がる場合は井戸性能を先に確認' },
    ...
    worksheet: { eyebrow: '現場評価シート', title: '基準値と現在性能を比較', intro: '揚水量、時間、条件が近い測定値を使います。結果をもとに、先に調べる項目を技術者と相談してください。', baseline: '基準値', current: '現在値', flow: '流量', staticLevel: '井口からの静水位', pumpingLevel: '井口からの動水位', sandFlag: '異常な砂・濁りあり', qualityFlag: '明確な水質・塩分変化あり', resultTitle: '優先確認経路', metrics: { flowChange: '流量変化', staticShift: '静水位の深化', drawdownChange: '水位低下量の変化' }, pathways: {
      sand: { title: '砂と井戸健全性を優先', text: '砂が機器を損傷する前に流量を下げ、時間別採水を行います。', next: '砂量と井戸深度を測り、証拠に応じカメラ調査を検討します。' }, quality: { title: '水質を優先', text: '影響の大きい用途から隔離し、適切な採水と分析で確認します。', next: 'EC/TDS、pH、濁度、運転時間、用途別分析を記録します。' }, aquifer: { title: '帯水層水位の変化を先に調査', text: '静水位の大幅な深化は季節、涵養、周辺揚水の影響が考えられます。', next: '周辺井戸・降雨を確認し、制御した揚水/回復試験を行います。' }, well: { title: '井戸・取水部効率を調査', text: '揚水前の水位が同程度で、流量が減り揚水中の水位低下が増えた場合、井戸内の損失増加が考えられます。原因の確認が必要です。', next: '揚水量を水位低下量で割った値を比較し、付着物や堆積物を調べてから詳しい調査と回復方法を計画します。' }, equipment: { title: 'ポンプ・配管・弁・電気を先に確認', text: '流量低下でも水位と低下量は同程度です。', next: '圧力、電流、電圧、弁位置を記録し、漏れ・摩耗を確認します。' }, stable: { title: '入力値に大きな変化なし', text: '測定方法と条件を確認し、傾向監視を続けます。', next: '性能、運転時間、水質、周辺イベントを毎月記録します。' },
...
export default function GroundwaterCaseStudies({ locale = 'th', localized = false }: { locale?: LocalizedLocale; localized?: boolean }) {
  ...
  const [baselineFlowDraft, setBaselineFlow] = useState('30')
  const baselineFlow = validateNumericDraft(baselineFlowDraft, { min: 0 }).value ?? 0
  const [currentFlowDraft, setCurrentFlow] = useState('20')
  const currentFlow = validateNumericDraft(currentFlowDraft, { min: 0 }).value ?? 0
  const [baselineStaticDraft, setBaselineStatic] = useState('12')
  const baselineStatic = validateNumericDraft(baselineStaticDraft, { min: 0 }).value ?? 0
  const [currentStaticDraft, setCurrentStatic] = useState('13')
  const currentStatic = validateNumericDraft(currentStaticDraft, { min: 0 }).value ?? 0
  const [baselinePumpingDraft, setBaselinePumping] = useState('25')
  const baselinePumping = validateNumericDraft(baselinePumpingDraft, { min: 0 }).value ?? 0
  const [currentPumpingDraft, setCurrentPumping] = useState('31')
  const currentPumping = validateNumericDraft(currentPumpingDraft, { min: 0 }).value ?? 0
  ...
  const baselinePumpingError = baselinePumping < baselineStatic ? learningFeedback[locale].pumpingLevel : undefined
  const currentPumpingError = currentPumping < currentStatic ? learningFeedback[locale].pumpingLevel : undefined
  const inputsValid = [baselineFlowDraft, currentFlowDraft, baselineStaticDraft, currentStaticDraft, baselinePumpingDraft, currentPumpingDraft].every((raw) => !validateNumericDraft(raw, { min: 0 }).error) && !baselinePumpingError && !currentPumpingError
  ...
  return (
    <article className="gcs-course">
      <details className="gcs-navigation">
        <summary>{copy.jumpLabel}</summary>
        <nav className="gcs-chapter-nav" aria-label={copy.jumpLabel}><div>{chapterIds.map((chapter) => { const Icon = chapterIcons[chapter]; return <a href={`#gcs-${chapter}`} key={chapter} onClick={(event) => { if (event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return; event.currentTarget.closest('details')?.removeAttribute('open'); document.getElementById(`gcs-${chapter}`)?.focus({ preventScroll: true }) }}><Icon aria-hidden="true" />{copy.chapters[chapter]}</a> })}</div></nav>
      </details>

      <section className="gcs-module" id="gcs-triage" tabIndex={-1}>
        <header className="gcs-heading"><div><p>{copy.triage.eyebrow}</p><h2><span><ShieldAlert aria-hidden="true" /></span>{copy.triage.title}</h2><div>{copy.triage.intro}</div></div></header>
        <div className="gcs-symptom-explorer">
          ...
          <div className="gcs-symptom-tabs" role="group" aria-label={copy.triage.selectorTitle}>{(Object.keys(copy.triage.labels) as SymptomId[]).map((id) => { const Icon = symptomIcons[id]; return <button type="button" id={`gcs-symptom-${id}`} aria-pressed={symptom === id} aria-controls="gcs-symptom-result" className={symptom === id ? 'is-active' : ''} onClick={() => setSymptom(id)} key={id}><Icon aria-hidden="true" /><span>{copy.triage.labels[id]}</span></button> })}</div>
          <aside className="gcs-stop-card"><div><h3><AlertOctagon aria-hidden="true" />{copy.triage.stopTitle}</h3><ul>{copy.triage.stopItems.map((item) => <li key={item}>{item}</li>)}</ul></div></aside>
          <div className="gcs-symptom-panel" id="gcs-symptom-result" role="region" aria-labelledby={`gcs-symptom-${symptom}`} aria-live="polite"><div className="gcs-signal"><Activity aria-hidden="true" /><p>{selectedSymptom.signal}</p></div><div className="gcs-action-grid"><div className="is-good"><p><strong><CheckCircle2 aria-hidden="true" />{copy.triage.first}</strong>{selectedSymptom.first}</p></div><div className="is-warning"><p><strong><AlertTriangle aria-hidden="true" />{copy.triage.avoid}</strong>{selectedSymptom.avoid}</p></div><div className="is-alert"><p><strong><ShieldAlert aria-hidden="true" />{copy.triage.escalate}</strong>{selectedSymptom.escalate}</p></div></div><div className="gcs-detail-grid"><section><h3><Search aria-hidden="true" />{copy.triage.likely}</h3><ul>{selectedSymptom.causes.map((item) => <li key={item}>{item}</li>)}</ul></section><section><h3><ClipboardCheck aria-hidden="true" />{copy.triage.collect}</h3><ul>{selectedSymptom.collect.map((item) => <li key={item}>{item}</li>)}</ul></section></div></div>
        </div>
        <figure className="gcs-comparison-figure"><Image src="/images/learning/groundwater-case-studies/healthy-vs-failing-well.webp" alt={copy.visualAlt.comparison} width={1536} height={1024} sizes="(width <= 900px) 100vw, 1280px" /><figcaption>{copy.visualCaption.comparison}</figcaption></figure>
      </section>

      <details className="gcs-course-details">
        <summary>{copy.outcomesTitle}</summary>
        <div className="gcs-overview">
          <div className="gcs-summary"><div><span>{copy.summary.duration}</span><strong><History aria-hidden="true" />{copy.summary.durationValue}</strong></div><div><span>{copy.summary.level}</span><strong><Gauge aria-hidden="true" />{copy.summary.levelValue}</strong></div><div><span>{copy.summary.cases}</span><strong><BookOpenCheck aria-hidden="true" />{copy.summary.casesValue}</strong></div></div>
          <div className="gcs-outcomes"><ul>{copy.outcomes.map((item) => <li key={item}><CheckCircle2 aria-hidden="true" />{item}</li>)}</ul></div>
        </div>
      </details>

      <section className="gcs-module" id="gcs-workflow" tabIndex={-1}>
        <header className="gcs-heading"><div><p>{copy.workflow.eyebrow}</p><h2><span><Search aria-hidden="true" /></span>{copy.workflow.title}</h2><div>{copy.workflow.intro}</div></div></header>
        <div className="gcs-field-layout"><figure><Image src="/images/learning/groundwater-case-studies/field-diagnostic-team.webp" alt={copy.visualAlt.field} width={1536} height={1024} sizes="(width <= 900px) 100vw, 760px" /><figcaption>{copy.visualCaption.field}</figcaption></figure><aside><h3><Info aria-hidden="true" />{copy.workflow.principleTitle}</h3><p>{copy.workflow.principle}</p></aside></div>
        <div className="gcs-workflow">{copy.workflow.steps.map((step, index) => <article key={step.title}><div><h3><span>{String(index + 1).padStart(2, '0')}</span>{step.title}</h3><p>{step.detail}</p><small><strong>{copy.workflow.outputLabel}:</strong> {step.output}</small></div></article>)}</div>
      </section>

      <section className="gcs-module" id="gcs-cases" tabIndex={-1}>
        <header className="gcs-heading"><div><p>{copy.cases.eyebrow}</p><h2><span><BookOpenCheck aria-hidden="true" /></span>{copy.cases.title}</h2><div>{copy.cases.intro}</div></div></header>
        <div className="gcs-cases">{copy.cases.items.map((item, index) => <details key={item.title} open={index === 0}><summary><div><small>{item.setting}</small><h3><span>{String(index + 1).padStart(2, '0')}</span>{item.title}</h3></div><ArrowRight aria-hidden="true" /></summary><div className="gcs-case-body"><dl><div><dt>{copy.cases.labels.symptom}</dt><dd>{item.symptom}</dd></div><div><dt>{copy.cases.labels.evidence}</dt><dd>{item.evidence}</dd></div><div><dt>{copy.cases.labels.finding}</dt><dd>{item.finding}</dd></div><div><dt>{copy.cases.labels.action}</dt><dd>{item.action}</dd></div><div><dt>{copy.cases.labels.verify}</dt><dd>{item.verify}</dd></div></dl><aside><div><strong><CheckCircle2 aria-hidden="true" />{copy.cases.labels.lesson}</strong><p>{item.lesson}</p></div></aside></div></details>)}</div>
      </section>

      <section className="gcs-module" id="gcs-worksheet" tabIndex={-1}>
        <header className="gcs-heading"><div><p>{copy.worksheet.eyebrow}</p><h2><span><BarChart3 aria-hidden="true" /></span>{copy.worksheet.title}</h2><div>{copy.worksheet.intro}</div></div></header>
        <p className="gcs-example-note"><Info aria-hidden="true" />{worksheetExampleByLocale[locale]}</p>
        <div className="gcs-worksheet">
          <div className="gcs-worksheet-inputs"><div className="gcs-input-head"><span /> <strong>{copy.worksheet.baseline}</strong><strong>{copy.worksheet.current}</strong></div><div className="gcs-paired-row"><span>{copy.worksheet.flow}</span><NumericInput locale={locale} min={0} id="gcs-baseline-flow" label={`${copy.worksheet.flow} — ${copy.worksheet.baseline}`} unit="m³/h" value={baselineFlowDraft} onChange={setBaselineFlow} /><NumericInput locale={locale} min={0} id="gcs-current-flow" label={`${copy.worksheet.flow} — ${copy.worksheet.current}`} unit="m³/h" value={currentFlowDraft} onChange={setCurrentFlow} /></div><div className="gcs-paired-row"><span>{copy.worksheet.staticLevel}</span><NumericInput locale={locale} min={0} id="gcs-baseline-static" label={`${copy.worksheet.staticLevel} — ${copy.worksheet.baseline}`} unit="m" value={baselineStaticDraft} onChange={setBaselineStatic} /><NumericInput locale={locale} min={0} id="gcs-current-static" label={`${copy.worksheet.staticLevel} — ${copy.worksheet.current}`} unit="m" value={currentStaticDraft} onChange={setCurrentStatic} /></div><div className="gcs-paired-row"><span>{copy.worksheet.pumpingLevel}</span><NumericInput locale={locale} min={0} id="gcs-baseline-pumping" error={baselinePumpingError} label={`${copy.worksheet.pumpingLevel} — ${copy.worksheet.baseline}`} unit="m" value={baselinePumpingDraft} onChange={setBaselinePumping} /><NumericInput locale={locale} min={0} id="gcs-current-pumping" error={currentPumpingError} label={`${copy.worksheet.pumpingLevel} — ${copy.worksheet.current}`} unit="m" value={currentPumpingDraft} onChange={setCurrentPumping} /></div><div className="gcs-flags"><label><input type="checkbox" checked={hasSand} onChange={(event) => setHasSand(event.currentTarget.checked)} /><span><Check aria-hidden="true" /></span>{copy.worksheet.sandFlag}</label><label><input type="checkbox" checked={hasQualityChange} onChange={(event) => setHasQualityChange(event.currentTarget.checked)} /><span><Check aria-hidden="true" /></span>{copy.worksheet.qualityFlag}</label></div></div>
          {inputsValid ? <output className="gcs-result" aria-live="polite"><p>{copy.worksheet.resultTitle}</p><h3>{pathway.title}</h3><div className="gcs-metrics"><div><span>{copy.worksheet.metrics.flowChange}</span><strong>{formatter.format(flowChange)}%</strong></div><div><span>{copy.worksheet.metrics.staticShift}</span><strong>{formatter.format(staticShift)} m</strong></div><div><span>{copy.worksheet.metrics.drawdownChange}</span><strong>{formatter.format(drawdownChange)} m</strong></div></div><p>{pathway.text}</p><div className="gcs-next-step"><p><strong><ArrowRight aria-hidden="true" />{copy.worksheet.nextLabel}</strong>{pathway.next}</p></div><small><Info aria-hidden="true" />{copy.worksheet.screeningNote}</small><div className="gcs-report-actions"><button type="button" onClick={copyReport}>{copied ? <Check aria-hidden="true" /> : <Clipboard aria-hidden="true" />}{copied ? copy.worksheet.copied : copy.worksheet.copy}</button><button type="button" onClick={downloadReport}><Download aria-hidden="true" />{copy.worksheet.download}</button></div>{copyFailed && <p role="status">{learningFeedback[locale].copyFailed}</p>}</output> : <p className="learning-number-error" role="status">{learningFeedback[locale].incomplete}</p>}
        </div>
      </section>

      <section className="gcs-module" id="gcs-prevention" tabIndex={-1}>
        <header className="gcs-heading"><div><p>{copy.prevention.eyebrow}</p><h2><span><Wrench aria-hidden="true" /></span>{copy.prevention.title}</h2><div>{copy.prevention.intro}</div></div></header>
        <div className="gcs-prevention-grid"><div className="gcs-checklist"><div><h3>{copy.prevention.checklistTitle}</h3><strong>{checked.length}/{copy.prevention.checklist.length}</strong></div><div className="gcs-progress" aria-hidden="true"><span style={{ width: `${(checked.length / copy.prevention.checklist.length) * 100}%` }} /></div>{copy.prevention.checklist.map((item, index) => <label key={item} className={checked.includes(index) ? 'is-checked' : ''}><input type="checkbox" checked={checked.includes(index)} onChange={() => setChecked((current) => current.includes(index) ? current.filter((value) => value !== index) : [...current, index])} /><span><Check aria-hidden="true" /></span>{item}</label>)}<p role="status">{checked.length === copy.prevention.checklist.length ? copy.prevention.complete : `${copy.prevention.remaining}: ${copy.prevention.checklist.length - checked.length}`}</p></div><div className="gcs-records"><h3>{copy.prevention.recordsTitle}</h3>{copy.prevention.records.map((item, index) => { const Icon = index === 0 ? Droplets : index === 1 ? Zap : index === 2 ? FlaskConical : History; return <article key={item.title}><div><h4><Icon aria-hidden="true" />{item.title}</h4><p>{item.detail}</p></div></article> })}</div></div>
      </section>

      <section className="gcs-quiz" id="gcs-quiz" tabIndex={-1}><header><p>{copy.quiz.eyebrow}</p><h2>{copy.quiz.title}</h2><span>{copy.quiz.intro}</span></header><div>{copy.quiz.questions.map((question, questionIndex) => { const selected = answers[questionIndex]; const answered = selected !== undefined; return <fieldset key={question.question} id={`gcs-question-${questionIndex}`}><legend>{question.question}</legend>{question.choices.map((choice, choiceIndex) => <button type="button" key={choice} className={answered ? choiceIndex === question.correct ? 'is-correct' : selected === choiceIndex ? 'is-wrong' : '' : ''} aria-pressed={selected === choiceIndex} aria-describedby={answered ? `gcs-feedback-${questionIndex}` : undefined} onClick={() => setAnswers((current) => ({ ...current, [questionIndex]: choiceIndex }))}><span>{answered && choiceIndex === question.correct ? <CheckCircle2 aria-hidden="true" /> : null}</span>{choice}</button>)}<p id={`gcs-feedback-${questionIndex}`} role="status" aria-live="polite" className={answered ? selected === question.correct ? 'is-correct' : 'is-review' : 'learning-quiz-feedback-empty'}>{answered && <><strong>{selected === question.correct ? copy.quiz.correct : copy.quiz.review}:</strong> {question.explanation}</>}</p></fieldset> })}</div>{quizComplete && <div className="gcs-quiz-result" aria-live="polite"><div><span>{copy.quiz.score}</span><strong>{quizScore}/{copy.quiz.questions.length}</strong>{quizScore === copy.quiz.questions.length && <p>{copy.quiz.perfect}</p>}</div><button type="button" onClick={() => { setAnswers({}); document.querySelector<HTMLButtonElement>('#gcs-question-0 button')?.focus() }}><RefreshCcw aria-hidden="true" />{copy.quiz.retry}</button></div>}</section>
      ...
      <section className="gcs-next"><div><h2><Factory aria-hidden="true" />{copy.next.title}</h2><p>{copy.next.text}</p></div><nav><Link href={localLink('/learn/groundwater-calculator-tools')}>{copy.next.calculator}<ArrowRight aria-hidden="true" /></Link><Link href={localLink('/contact')}>{copy.next.contact}</Link></nav></section>
    </article>
  )
}
