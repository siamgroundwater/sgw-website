  ...
  return (
    <>
      <section className="cms-panel">
        <header className="cms-panel-header"><div><h2>{text('สิทธิ์เข้าถึง CMS', 'CMS access')}</h2><p>{text('กำหนดสิทธิ์เฉพาะเท่าที่แต่ละคนจำเป็น ระบบต้องมีผู้ดูแลที่เปิดใช้งานอย่างน้อยหนึ่งคนเสมอ', 'Assign the least access each person needs. At least one active administrator is always required.')}</p></div><button className="cms-button" type="button" onClick={() => show()}><Plus aria-hidden="true" />{text('เพิ่มผู้ใช้', 'Add user')}</button></header>
