...
const chapterIds: ChapterId[] = ['navigator', 'foundation', 'lifecycle', 'duties', 'library', 'penalties', 'quiz']
  ...
  th: {
    ...
    foundation: {
      ...
      layers: [
        ...
        { title: 'ประกาศ', text: 'กำหนดเขต/ความลึก มาตรฐานวิชาการ แบบ นบ. ผู้ควบคุมงาน และเขตวิกฤตการณ์', example: 'เกณฑ์ลึกเกิน 15 เมตรใช้กำหนดขอบเขตตามกฎหมายทั่วประเทศ ไม่ใช่ความลึกที่น้ำบาดาลตามธรรมชาติเริ่มเกิด' },
    ...
    lifecycle: {
      ...
      steps: [
        ...
        { title: 'จำแนกกิจกรรมและประเภทการใช้', text: 'แยกเจาะ ใช้ หรือระบาย และประเมินว่าการใช้อาจเข้าประเภทที่ 2–3 หรือไม่', evidence: 'ปริมาณน้ำเข้า–ออก ปริมาณใช้สูงสุด วัตถุประสงค์ และแผนบริหารจัดการน้ำ' },
        ...
        { title: 'เจาะและรายงานตามมาตรฐาน', text: 'บันทึกชั้นดิน/หิน ช่วงกรอง การพัฒนาบ่อ การทดสอบสูบ และตัวอย่างน้ำ', evidence: 'รายงานการเจาะและชั้นดินหิน ผลสูบทดสอบ และผลตรวจน้ำ' },
...
copyByLocale.ja = {
  ...
  foundation: { eyebrow: '階層順に読む', title: '5つの法令階層が連動します', intro: '地下水法だけでは不十分です。申請様式、技術基準、料金、手続は下位法令にあります。', exampleLabel: '例', layers: [{ title: '地下水法', text: '定義、権限、許可、義務、検査、不服申立て、罰則を定めます。', example: '第16条は事前許可を求めています。' }, { title: '水資源法', text: '公共水資源の第1～3種利用と、影響の大きい事業への追加管理を定めます。', example: '第2～3種には水管理計画が必要です。' }, { title: '省令', text: '利用区分、申請、許可期間、手数料、料金を定めます。', example: '申請書類と許可の有効期間。' }, { title: '告示', text: '区域・深度、技術基準、DGR様式、有資格監督者、危機区域を定めます。', example: '全国の15メートル超という基準は法律の適用範囲を示します。自然の地下水が現れ始める深度ではありません。' }, { title: '手続規程と許可条件', text: '現地検査、譲渡、更新、電子手続、案件固有の条件を定めます。', example: '許可保持者は交付された条件に従います。' }], exactTitle: '着手前に知る主要条文', exactIntro: '以下は要約です。現行統合版でタイ語原文と改正注記を確認してください。', sections: [{ section: '第16条', title: '土地所有だけでは許可にならない', text: '地下水区域で地下水事業を行うには許可が必要です。' }, { section: '第18条', title: '3種類の許可は別々', text: '掘削、利用、井戸への排出は、それぞれ別の許可です。' }, { section: '第20条', title: '許可には期限がある', text: '法律が最長期間を定め、実際の期間は下位法令と交付許可で決まります。' }, { section: '第22～25/1条', title: '条件、掲示、料金', text: '許可条件、掲示・再交付、適用される利用料・保全料は継続義務です。' }, { section: '第26～27条', title: '廃止には届出と措置が必要', text: '廃止後15日以内に届け出て、法令と指示に従い埋戻しまたは設備撤去を行います。' }], sourceLink: '現行地下水法を開く' },
  ...
  lifecycle: { eyebrow: '敷地から埋戻しまで', title: '9段階の法令遵守サイクル', intro: '掘削後の後追いではなく、開始時から証拠と文書を工程に組み込みます。', evidenceLabel: '保管資料', steps: [{ title: '区域と深度を確認', text: '地下水区域・危機区域、管轄事務所、地域制限を確認します。', evidence: '座標、地図、土地権利、公式確認。' }, { title: '活動と用途を分類', text: '掘削、利用、排出を分け、第2～3種利用の可能性を確認します。', evidence: '入水量と使用水量、最大需要、用途、水管理計画。' }, { title: '有資格者と様式を準備', text: '有効な資格を持つ監督者と最新様式を使用します。', evidence: '資格証明、井戸設計、機械一覧、同意書。' }, { title: '掘削許可を取得', text: '申請は許可ではありません。着手前にすべての条件を確認します。', evidence: '交付され、現場に掲示した許可証。' }, { title: '基準どおりに掘削・報告', text: '地質、スクリーン、洗浄、揚水試験、試料を記録します。', evidence: '井戸記録、試験、分析結果。' }, { title: '利用許可を取得', text: '通常揚水の前に施工・利用情報を提出します。', evidence: '利用許可、許可水量、特別条件。' }, { title: '計量・報告', text: '検証可能なメーターを維持し、翌月7日までにNB.11を提出します。', evidence: 'メーター写真、記録、提出控え、受領証。' }, { title: '支払・更新・変更', text: '期限どおり支払い、満了前に更新し、変更・譲渡は事前承認を得ます。', evidence: '領収書、日程表、書面承認。' }, { title: '届出・埋戻し', text: '廃止時に届け出て、帯水層間の汚染を防ぐよう埋め戻します。', evidence: '届出、NB.12、写真、埋戻し報告。' }] },
...
export default function GroundwaterLawGuide({ locale = 'th', localized = false }: { locale?: LocalizedLocale; localized?: boolean }) {
  ...
  const [depthDraft, setDepth] = useState('18')
  const depth = validateNumericDraft(depthDraft, { min: 0 }).value
  ...
  const progress = useLearningProgress('law')
  ...
  return (
    <article className="gwl-guide">
      <aside className="gwl-disclaimer"><div><h2><Scale aria-hidden="true" />{copy.disclaimer.title}</h2><p>{copy.disclaimer.text}</p></div></aside>

      <section className="gwl-module" id="gwl-navigator" tabIndex={-1}>
        <header className="gwl-heading"><div><p>{copy.navigator.eyebrow}</p><h2><span><FileSearch aria-hidden="true" /></span>{copy.navigator.title}</h2><div>{copy.navigator.intro}</div></div></header>
        <div className="gwl-navigator"><form onSubmit={(event) => event.preventDefault()}><NumericInput id="gwl-depth" label={copy.navigator.depth} value={depthDraft} onChange={setDepth} locale={locale} unit="m" min={0} help={copy.navigator.depthHint} /><label><span>{copy.navigator.stage}</span><select value={stage} onChange={(event) => setStage(event.currentTarget.value as ProjectStage)}>{(Object.keys(copy.navigator.stages) as ProjectStage[]).map((value) => <option key={value} value={value}>{copy.navigator.stages[value]}</option>)}</select></label><label><span>{copy.navigator.use}</span><select value={useProfile} onChange={(event) => setUseProfile(event.currentTarget.value as UseProfile)}>{(Object.keys(copy.navigator.uses) as UseProfile[]).map((value) => <option key={value} value={value}>{copy.navigator.uses[value]}</option>)}</select></label><div className="gwl-navigator-flags"><label><input type="checkbox" checked={criticalZone} onChange={(event) => setCriticalZone(event.currentTarget.checked)} /><span><Check aria-hidden="true" /></span>{copy.navigator.critical}</label><label><input type="checkbox" checked={discharge} onChange={(event) => setDischarge(event.currentTarget.checked)} /><span><Check aria-hidden="true" /></span>{copy.navigator.discharge}</label></div></form><output aria-live="polite">{depth === null ? <p className="learning-number-error">{learningFeedback[locale].incomplete}</p> : <><p>{copy.navigator.resultTitle}</p><div>{navigatorResults.map((result, index) => <article key={result}><span>{index + 1}</span><p>{result}</p></article>)}</div><small><Info aria-hidden="true" />{copy.navigator.verify}</small></>}</output></div>
      </section>

      <nav className="gwl-chapter-nav" aria-label={copy.jumpLabel}><details><summary>{copy.jumpLabel}</summary><div>{chapterIds.map((chapter) => { const Icon = chapterIcons[chapter]; return <a href={`#gwl-${chapter}`} key={chapter} onClick={(event) => { if (event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return; event.currentTarget.closest('details')?.removeAttribute('open'); document.getElementById(`gwl-${chapter}`)?.focus({ preventScroll: true }) }}><Icon aria-hidden="true" />{copy.chapters[chapter]}</a> })}</div></details></nav>

      <section className="gwl-module" id="gwl-foundation" tabIndex={-1}>
        <header className="gwl-heading"><div><p>{copy.foundation.eyebrow}</p><h2><span><Landmark aria-hidden="true" /></span>{copy.foundation.title}</h2><div>{copy.foundation.intro}</div></div></header>
        <figure className="gwl-hero-figure"><Image src="/images/learning/groundwater-law/legal-hierarchy.webp" alt={copy.imageAlt.hierarchy} width={1536} height={1024} sizes="(width <= 900px) 100vw, 1280px" /><figcaption>{copy.imageCaption.hierarchy}</figcaption></figure>
        <div className="gwl-layers">{copy.foundation.layers.map((layer, index) => <article key={layer.title}><h3><span>{String(index + 1).padStart(2, '0')}</span>{layer.title}</h3><p>{layer.text}</p><small><strong>{copy.foundation.exampleLabel}:</strong> {layer.example}</small></article>)}</div>
        <div className="gwl-sections"><header><h3>{copy.foundation.exactTitle}</h3><p>{copy.foundation.exactIntro}</p></header><div>{copy.foundation.sections.map((item) => <article key={item.section}><h4><span>{item.section}</span>{item.title}</h4><p>{item.text}</p></article>)}</div><a href={groundwaterLawOfficialRepositories.acts} target="_blank" rel="noopener noreferrer">{copy.foundation.sourceLink}<ExternalLink aria-hidden="true" /></a></div>
      </section>

      <section className="gwl-module" id="gwl-lifecycle" tabIndex={-1}>
        <header className="gwl-heading"><div><p>{copy.lifecycle.eyebrow}</p><h2><span><HardHat aria-hidden="true" /></span>{copy.lifecycle.title}</h2><div>{copy.lifecycle.intro}</div></div></header>
        ...
        <div className="gwl-lifecycle">{copy.lifecycle.steps.map((step, index) => <article key={step.title}><div><h3><span>{String(index + 1).padStart(2, '0')}</span>{step.title}</h3><p>{step.text}</p><small><strong><FileCheck2 aria-hidden="true" />{copy.lifecycle.evidenceLabel}:</strong> {step.evidence}</small></div></article>)}</div>
      </section>

      <section className="gwl-module" id="gwl-duties" tabIndex={-1}>
        <header className="gwl-heading"><div><p>{copy.duties.eyebrow}</p><h2><span><ClipboardCheck aria-hidden="true" /></span>{copy.duties.title}</h2><div>{copy.duties.intro}</div></div></header>
        <div className="gwl-duties">{copy.duties.items.map((item, index) => { const Icon = index === 0 ? FileCheck2 : index === 1 ? Gauge : index === 2 ? Wrench : index === 3 ? CalendarClock : index === 4 ? Landmark : index === 5 ? Droplets : index === 6 ? History : ShieldCheck; return <article key={item.title}><h3><Icon aria-hidden="true" />{item.title}</h3><p>{item.rule}</p><dl><div><dt>{copy.duties.timingLabel}</dt><dd>{item.timing}</dd></div><div><dt>{copy.duties.sourceLabel}</dt><dd>{item.source}</dd></div></dl></article> })}</div>
        <aside className="gwl-deadlines"><div><h3><CalendarClock aria-hidden="true" />{copy.duties.deadlineTitle}</h3><ul>{copy.duties.deadlineItems.map((item) => <li key={item}>{item}</li>)}</ul></div></aside>
        <div className="gwl-checklist"><header><div><h3>{copy.checklist.title}</h3><p>{copy.checklist.intro}</p></div><strong>{checked.length}/{copy.checklist.items.length}</strong></header><div className="gwl-progress" aria-hidden="true"><span style={{ width: `${(checked.length / copy.checklist.items.length) * 100}%` }} /></div><div>{copy.checklist.items.map((item, index) => <label key={item} className={checked.includes(progress.ids[index]) ? 'is-checked' : ''}><input type="checkbox" disabled={!progress.ready} checked={checked.includes(progress.ids[index])} onChange={() => progress.toggle(progress.ids[index])} /><span><Check aria-hidden="true" /></span>{item}</label>)}</div><p role="status">{checked.length === copy.checklist.items.length ? copy.checklist.complete : `${copy.checklist.remaining}: ${copy.checklist.items.length - checked.length}`}</p></div>
        <LearningProgress locale={locale} checklist="law" progress={progress} />
      </section>

      <section className="gwl-module" id="gwl-library" tabIndex={-1}>
        <header className="gwl-heading"><div><p>{copy.library.eyebrow}</p><h2><span><Search aria-hidden="true" /></span>{copy.library.title}</h2><div>{copy.library.intro}</div></div></header>
        <p className="gwl-library-scope">{copy.disclaimer.scope}</p>
        {locale !== 'th' && <aside className="gwl-language-note"><Info aria-hidden="true" />{copy.library.officialThaiNote}</aside>}
        <div className="gwl-library-controls"><label htmlFor="gwl-law-search"><span>{copy.library.searchLabel}</span><div><Search aria-hidden="true" /><input id="gwl-law-search" type="search" value={query} onChange={(event) => setQuery(event.currentTarget.value)} placeholder={copy.library.searchPlaceholder} /></div></label><div className="gwl-filters"><span><Filter aria-hidden="true" />{copy.library.filtersLabel}</span><div role="group" aria-label={copy.library.filtersLabel}><button type="button" aria-pressed={category === 'all'} className={category === 'all' ? 'is-active' : ''} onClick={() => setCategory('all')}>{copy.library.all}</button>{(Object.keys(copy.library.categories) as GroundwaterLawCategory[]).map((value) => <button type="button" aria-pressed={category === value} className={category === value ? 'is-active' : ''} onClick={() => setCategory(value)} key={value}>{copy.library.categories[value]}</button>)}</div></div><label className="gwl-reference-toggle"><input type="checkbox" checked={showReference} onChange={(event) => setShowReference(event.currentTarget.checked)} /><span><Check aria-hidden="true" /></span>{copy.library.showReference}</label></div>
        <div className="gwl-library-count" role="status"><strong>{filteredDocuments.length}</strong> {copy.library.results}</div>
        ...
        <aside className="gwl-master-library"><div><h3><Landmark aria-hidden="true" />{copy.library.repositoriesTitle}</h3><p>{copy.library.repositoriesText}</p></div><a href={groundwaterLawOfficialRepositories.master} target="_blank" rel="noopener noreferrer">{copy.library.masterLink}<ExternalLink aria-hidden="true" /></a></aside>
      </section>

      <section className="gwl-module" id="gwl-penalties" tabIndex={-1}>
        <header className="gwl-heading"><div><p>{copy.penalties.eyebrow}</p><h2><span><Gavel aria-hidden="true" /></span>{copy.penalties.title}</h2><div>{copy.penalties.intro}</div></div></header>
        <div className="gwl-penalties">{copy.penalties.items.map((item) => <article key={item.section}><h3><span>{item.section}</span>{item.title}</h3><p>{item.text}</p></article>)}</div><aside className="gwl-penalty-warning"><div><h3><AlertTriangle aria-hidden="true" />{copy.penalties.warningTitle}</h3><p>{copy.penalties.warningText}</p></div></aside>
      </section>

      <section className="gwl-quiz" id="gwl-quiz" tabIndex={-1}><header><p>{copy.quiz.eyebrow}</p><h2>{copy.quiz.title}</h2><span>{copy.quiz.intro}</span></header><div>{copy.quiz.questions.map((question, questionIndex) => { const selected = answers[questionIndex]; const answered = selected !== undefined; return <fieldset key={question.question} id={`gwl-question-${questionIndex}`}><legend>{question.question}</legend>{question.choices.map((choice, choiceIndex) => <button type="button" key={choice} className={answered ? choiceIndex === question.correct ? 'is-correct' : selected === choiceIndex ? 'is-wrong' : '' : ''} aria-pressed={selected === choiceIndex} aria-describedby={answered ? `gwl-feedback-${questionIndex}` : undefined} onClick={() => setAnswers((current) => ({ ...current, [questionIndex]: choiceIndex }))}><span>{answered && choiceIndex === question.correct ? <CheckCircle2 aria-hidden="true" /> : null}</span>{choice}</button>)}<p id={`gwl-feedback-${questionIndex}`} role="status" aria-live="polite" className={answered ? selected === question.correct ? 'is-correct' : 'is-review' : 'learning-quiz-feedback-empty'}>{answered && <><strong>{selected === question.correct ? copy.quiz.correct : copy.quiz.review}:</strong> {question.explanation}</>}</p></fieldset> })}</div>{quizComplete && <div className="gwl-quiz-result" aria-live="polite"><div><span>{copy.quiz.score}</span><strong>{quizScore}/{copy.quiz.questions.length}</strong>{quizScore === copy.quiz.questions.length && <p>{copy.quiz.perfect}</p>}</div><button type="button" onClick={() => { setAnswers({}); document.querySelector<HTMLButtonElement>('#gwl-question-0 button')?.focus() }}><RefreshCcw aria-hidden="true" />{copy.quiz.retry}</button></div>}</section>

      <p className="gwl-review-date">{copy.summary.reviewed}: {copy.summary.reviewedValue}</p>
      ...
      <section className="gwl-next"><div><h2><MapPinned aria-hidden="true" />{copy.next.title}</h2><p>{copy.next.text}</p></div><nav><Link href={localLink('/contact')}>{copy.next.contact}<ArrowRight aria-hidden="true" /></Link><Link href={localLink('/learn/groundwater-basics-thailand')}>{copy.next.basics}</Link></nav></section>
    </article>
  )
}
